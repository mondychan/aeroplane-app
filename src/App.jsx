...
function CredentialEditor(){return <><CartoKeyEditor/><OpenSkyCredentialEditor/></>}

function CartoKeyEditor(){
 const[status,setStatus]=useState({configured:false}),[apiKey,setApiKey]=useState(''),[message,setMessage]=useState(''),[saving,setSaving]=useState(false)
 useEffect(()=>{fetch('/api/credentials/carto').then(r=>r.json()).then(setStatus)},[])
 const saveKey=async()=>{setSaving(true);setMessage('');try{const r=await fetch('/api/credentials/carto',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({apiKey})}),body=await r.json();if(!r.ok)throw Error(body.error||'Uložení CARTO klíče selhalo');setStatus({...body,apiKey});setApiKey('');window.dispatchEvent(new Event('carto-key-changed'));setMessage('CARTO klíč byl uložen.')}catch(e){setMessage(e.message)}finally{setSaving(false)}}
 const remove=async()=>{const r=await fetch('/api/credentials/carto',{method:'DELETE'});setStatus(await r.json());window.dispatchEvent(new Event('carto-key-changed'));setMessage('CARTO klíč byl odstraněn.')}
 const file=async e=>{const selected=e.target.files?.[0];if(selected){setApiKey((await selected.text()).trim());setMessage(`Načten soubor ${selected.name}. Dosud nebyl odeslán.`)}}
 return <section className="credentials-box"><header><div><ShieldCheck/><span>CARTO BASEMAPS API KEY</span></div><b className={status.configured?'configured':''}>{status.configured?`Nastaveno · ${status.maskedApiKey}`:'Nenastaveno'}</b></header><p>Klíč se uloží šifrovaně na serveru a použije se pro CARTO mapové dlaždice.</p><div className="credential-grid"><label className="credential-paste">CARTO API key<input type="password" autoComplete="new-password" value={apiKey} onChange={e=>setApiKey(e.target.value)} placeholder="cb1_..."/></label><label className="file-pick"><FileJson/><span>Nahrát textový soubor</span><input type="file" accept=".txt,.key,text/plain" onChange={file}/></label></div>{message&&<div className="credential-message">{message}</div>}<footer><button disabled={!status.configured||saving} onClick={remove}><Trash2/>Odstranit</button><button className="primary" disabled={saving||!apiKey.trim()} onClick={saveKey}>{saving?'Ukládám…':'Uložit CARTO klíč'}</button></footer></section>
}

function OpenSkyCredentialEditor(){
