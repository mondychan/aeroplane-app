...
export class JsonStore {
  ...
  async addHistory(entry) { const duplicate = this.history.find((item) => item.icao24 === entry.icao24 && Date.now() - new Date(item.seenAt).getTime() < 10 * 60 * 1000); if (duplicate) return; this.history.unshift(entry); this.history = this.history.slice(0, 500); await this.atomicWrite(this.historyFile, this.history) }
  ...
  getStats() {
    ...
    return { total: this.history.length, airlines: count('airline'), aircraftTypes: count('aircraftType') }
  }
}
